...
*Last updated: 2026-08-08. Added a "Change phase" link + current-phase label to the check-in flow header, shown on every question and the review screen (decision 19) — previously a returning user with a phase already saved had no way to see or change it without leaving to the dashboard. Fixed a staleness bug this surfaced: `showView()` wasn't refreshing the flow's chrome on return, so the label could show the old phase after changing it. Reverted an over-eager same-session change that had split "Base / Taper" into two separate phases — reconfirmed as one combined option, per the picker's original design. Prior update (2026-08-07): removed the manual "mark complete" button from Warm up and both Strength mini-apps in favour of auto-detect-only completion (decision 17); rebranded `--blue`/`--swim` to the favicon's `#0022FF` (decision 16); corrected the documented type scale to match shipped CSS, 23px titles/14px eyebrows not 26px/9–10px (decision 18); added the splash screen (decision 15) and a full design-guide review (light-only theme, real 4-tab nav, new component tiles). See decision 14 and "Two codebases" intro for the current-vs-target split. Owner: Justin. Update this file whenever a decision changes, or when the target architecture moves from scaffold to active development.*
