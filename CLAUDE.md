...
*Last updated: 2026-08-11. Added a coloured phase pill to Month-view week cards (decision 21), replacing the plain-text "Week N · Phase" label — introduced dedicated `--phase-{base,build,peak}` tokens and retired the never-implemented `--base`/`--build`/`--peak`/`--taper`-map-to-discipline-colour plan from the Colour tokens block. Fixed grouped check-in pages clipping their last option behind the sticky footer at full scroll (decision 20) and restored the missing "Moderate / tempo" intensity option. Prior update (2026-08-08): added a "Change phase" link + current-phase label to the check-in flow header, shown on every question and the review screen (decision 19) — previously a returning user with a phase already saved had no way to see or change it without leaving to the dashboard. Fixed a staleness bug this surfaced: `showView()` wasn't refreshing the flow's chrome on return, so the label could show the old phase after changing it. Reverted an over-eager same-session change that had split "Base / Taper" into two separate phases — reconfirmed as one combined option, per the picker's original design. See decision 14 and "Two codebases" intro for the current-vs-target split. Owner: Justin. Update this file whenever a decision changes, or when the target architecture moves from scaffold to active development.*
