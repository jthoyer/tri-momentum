...
1. **Read `CHANGELOG.md` first — then follow its per-agent index.** That file is the current month only; an agent's prior entry may sit in `changelog/YYYY-MM.md` or `CHANGELOG-ARCHIVE.md`, and the index at the top says which. Establish who was reviewed, when, and whether the review had a work sample. A heading naming an agent is not necessarily a review of it — creation and batch entries count too. Do not re-litigate applied findings.
...
**The ledger is split by month, so size is no longer your problem.** `CHANGELOG.md` holds the **current month only**; closed months sit verbatim in `changelog/YYYY-MM.md`, and everything moved out by the old retention sweeps is in `CHANGELOG-ARCHIVE.md`. The per-agent index at the top of `CHANGELOG.md` says where each agent's entries live — read that before hunting. **Do not sweep, trim or archive anything on size grounds:** the top-2-per-agent and top-4-library rules were retired on 2026-09-13 because the split does their job structurally. If a single month ever stops fitting one `Read`, split that month by fortnight; never delete an entry to make room.
